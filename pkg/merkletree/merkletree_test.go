...
func prepareVerify(t *testing.T, dataSize int64, hashAlgorithm int, dataAndTreeInSameFile bool, isSymlink bool, verifyStart int64, verifySize int64, out io.Writer) ([]byte, VerifyParams) {
	t.Helper()
	...
	if isSymlink {
		genParams.Symlink = defaultSymlink
	}
...
func TestVerifyUnmodifiedSymlink(t *testing.T) {
	testCases := []struct {
		name                  string
		hashAlgorithm         int
		dataAndTreeInSameFile bool
